...
它不修改 scheduler、placer、quota 或 preemption cost。新账本补丁前后的五类既有 Spot CSV 完全一致；新增两类 CSV 只观察 run-start 与 guarantee 状态转换。FIFO 则正是触发 estimator 数值缺陷的路径。补丁基于 GPL-3.0-only 的 GFS；公开仓库保留上游归属、完整 GPL-3.0 文本和对应 patch。SchedNav 第一方代码采用 MIT License，根目录 MIT 不重新许可 GFS patch。
...
- SchedNav 第一方代码采用 MIT License；GFS patch 保持 GPL-3.0-only，AgentTeams patch 保持 Apache-2.0，第三方边界见 `THIRD_PARTY_NOTICES.md`。
